...
// For anchor links to scroll with right offset
...
let anchors = document.querySelectorAll('.anchor-collapse__item a');
...
// To Top
document.querySelectorAll('.to-top__link').forEach(el => {
...
// Sticky top
let navbar = document.getElementById("navFixed");
if(navbar) {
...
// Sliders
if(document.querySelector('.category-slider__list')) {
  let categorySlider = new Splide( '.category-slider__list', {
    type   : 'slide',
    drag   : 'free',
    autoWidth: true,
    perPage: 1,
...
if(document.querySelector('.cards-slider__list')) {
  let topSlider = new Splide( '.cards-slider__list', {
    type   : 'slide',
    drag   : true,
    gap: '10px',
    perPage: 1,
...
if(document.querySelector('.cards-slider__list_short')) {
  
  let sliders = document.querySelectorAll('.cards-slider__list_short');
  sliders.forEach(el => {
...
function modalClose(btn) {
  document.querySelectorAll(btn).forEach(el => {
    el.addEventListener('click', (evt) => {
      evt.preventDefault();
      evt.target.closest('.modal.popup-modal').style.display = 'none';
    })
  });
}
